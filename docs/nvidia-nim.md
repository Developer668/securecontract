...
The assistant loads the authenticated NVIDIA model catalog and groups it into fast/non-reasoning and reasoning options. Catalog visibility does not guarantee that every model is enabled for every NVIDIA account, so an unavailable selection safely falls back to deterministic, evidence-backed retrieval rather than inventing an answer. The default is `deepseek-ai/deepseek-v4-flash-0731`, which was verified against this local setup.
