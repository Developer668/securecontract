...
export class NvidiaNimProvider implements CopilotProvider {
  ...
  async chat(request:{question:string;opportunity:Opportunity;relatedOpportunities?:Opportunity[];vendor?:VendorProfile;workspace?:ApplicationContext}) {
    ...
    const response=await this.fetcher(`${this.config.baseUrl}/chat/completions`,{method:'POST',headers:{Authorization:`Bearer ${this.config.apiKey}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(45_000),body:JSON.stringify({model:this.config.model,temperature:0.1,max_tokens:700,response_format:{type:'json_object'},messages:[{role:'system',content:secureContractSystemPrompt},{role:'user',content:JSON.stringify({question:request.question,selectedContract:context,searchCandidates:related,instructions:'For a search request, recommend only records in searchCandidates. Use the selected contract JSON and evidence for a selected-contract request. Return answer, evidenceFields, opportunityIds, and draft as JSON.'})}]})});
