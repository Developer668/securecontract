...
export class NvidiaNimProvider implements CopilotProvider {
  ...
  async chat(request:{question:string;opportunity:Opportunity;relatedOpportunities?:Opportunity[];vendor?:VendorProfile;workspace?:ApplicationContext}) {
    ...
    const related=(request.relatedOpportunities??[]).slice(0,5).map((item)=>({id:item.id,title:item.titleOriginal,buyer:item.buyerOriginal,status:item.status,country:item.countryName,deadline:item.submissionDueAt,sourceUrl:item.sourceUrl,detailUrl:item.detailUrl,description:(item.descriptionEnglish??item.descriptionOriginal??'').slice(0,1200),evidence:item.evidence.slice(0,4).map(({fieldName,normalizedValue,sourceUrl})=>({fieldName,normalizedValue,sourceUrl}))}));
    const response=await this.fetcher(`${this.config.baseUrl}/chat/completions`,{method:'POST',headers:{Authorization:`Bearer ${this.config.apiKey}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(20_000),body:JSON.stringify({model:this.config.model,temperature:0.1,max_tokens:160,response_format:{type:'json_object'},messages:[{role:'system',content:secureContractSystemPrompt},{role:'user',content:JSON.stringify({question:request.question,selectedContract:context,searchCandidates:related,instructions:'For a search request, recommend only records in searchCandidates. Use the selected contract JSON and evidence for a selected-contract request. Return answer, evidenceFields, opportunityIds, and draft as JSON. Do not reveal chain-of-thought or include a reasoning trace; answer concisely.'})}]})});
