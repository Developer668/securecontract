...
type Message = {id:string;role:'user'|'assistant';content:string;evidenceIds?:string[]};
...
function OpportunityCard({item,onOpen}:{item:FundingOpportunity;onOpen:()=>void}){
  return <button className="funding-row" onClick={onOpen}>
    <span className="match-score">{item.match.score}<small>match</small></span>
...
function Detail({item,onClose}:{item:FundingOpportunity;onClose:()=>void}){
  return <div className="dialog-backdrop" onMouseDown={(e)=>e.target===e.currentTarget&&onClose()}><aside className="detail-dialog" aria-label="Funding opportunity detail">
    ...
    <div className="detail-summary"><strong>{item.match.score}% match</strong><span>{item.match.eligibility.replaceAll('_',' ')}</span><span>{item.amountText}</span><span>{date(item.deadline)}</span></div>
...
function Discovery({profile,items,onSources}:{profile:LabProfile;items:FundingOpportunity[];onSources:()=>void}){
  ...
  return <main className="workspace">
    <header className="workspace-head"><div><span className="step">Live funding · US biomedical</span><h1>Good morning, {profile.name}.</h1><p>{items.length?`${items.length} opportunities passed the Bright Data quality gate.`:'No live records yet. Run the source portfolio to begin matching.'}</p></div><button className="primary" onClick={onSources}><Pulse/>Run collectors</button></header>
...
function Guide({items}:{items:FundingOpportunity[]}){
  const [messages,setMessages]=useState<Message[]>([{id:'welcome',role:'assistant',content:'Ask me to compare the live funding records against your lab. I will keep unsupported eligibility uncertain and link consequential conclusions to collected passages.'}]);
  const [question,setQuestion]=useState('');const [busy,setBusy]=useState(false);
  const evidence=new Map(items.flatMap((item)=>item.evidence).map((entry)=>[entry.id,entry]));
  const send=async()=>{const prompt=question.trim();if(!prompt||busy)return;setQuestion('');setBusy(true);setMessages((current)=>[...current,{id:crypto.randomUUID(),role:'user',content:prompt}]);const response=await fetch('/api/funding/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:prompt,opportunityIds:[]})});const body=await response.json() as {answer?:string;error?:string;evidenceIds?:string[]};setMessages((current)=>[...current,{id:crypto.randomUUID(),role:'assistant',content:body.answer??body.error??'The guide could not answer.',evidenceIds:body.evidenceIds}]);setBusy(false)};
  return <main className="chat-page"><header><Brain/><div><strong>FundingSecured Guide</strong><small>NVIDIA NIM · live evidence only</small></div></header><section>{messages.map((message)=><article key={message.id} className={message.role}><span>{message.role==='assistant'?<Logo/>:'You'}</span><div><p>{message.content}</p>{message.evidenceIds?.length?<footer>{message.evidenceIds.map((id,index)=>{const entry=evidence.get(id);return entry?<a key={id} href={entry.sourceUrl} target="_blank" rel="noreferrer">Source {index+1} · {entry.field}</a>:null})}</footer>:null}</div></article>)}{busy&&<p className="thinking"><SpinnerGap className="spin"/>Reading live passages…</p>}</section><form onSubmit={(e)=>{e.preventDefault();void send()}}><textarea aria-label="Ask FundingSecured" value={question} onChange={(e)=>setQuestion(e.target.value)} placeholder={items.length?'Compare the strongest live opportunities…':'Run collectors before asking for live matches…'} disabled={!items.length}/><button disabled={!items.length||busy||question.trim().length<2}><PaperPlaneTilt/></button></form></main>;
}

export default function App(){
  const [view,setView]=useState<View>('discover');const [profile,setProfile]=useState<LabProfile|null>(null);const [items,setItems]=useState<FundingOpportunity[]>([]);const [sources,setSources]=useState<ClientSource[]>([]);const [events,setEvents]=useState<FundingEvent[]>([]);const [configured,setConfigured]=useState(false);const [loading,setLoading]=useState(true);
  const refresh=useCallback(async()=>{const [p,o,s,e]=await Promise.all([fetch('/api/funding/profile').then((r)=>r.json()),fetch('/api/funding/opportunities').then((r)=>r.json()),fetch('/api/funding/sources').then((r)=>r.json()),fetch('/api/funding/event-history').then((r)=>r.json())]);setProfile(p.data);setItems(o.data);setSources(s.data);setConfigured(s.brightDataConfigured);setEvents(e.data);setLoading(false)},[]);
  ...
  useEffect(()=>{const stream=new EventSource('/api/funding/events');stream.addEventListener('funding',(event)=>{const value=JSON.parse((event as MessageEvent).data) as FundingEvent;setEvents((current)=>[value,...current.filter((item)=>item.id!==value.id)])});return()=>stream.close()},[]);
  if(loading)return <main className="loading"><Logo/><span>Loading FundingSecured</span></main>;
  const go=(next:View)=>setView(next);
  return <div className="app"><header className="topbar"><button className="brand" onClick={()=>go('discover')}><Logo/>FundingSecured</button><nav>{profile&&<button className={view==='discover'?'active':''} onClick={()=>go('discover')}>Funding</button>}<button className={view==='profile'?'active':''} onClick={()=>go('profile')}>Lab profile</button><button className={view==='sources'?'active':''} onClick={()=>go('sources')}>Sources</button>{profile&&<button className={view==='guide'?'active':''} onClick={()=>go('guide')}>Ask AI</button>}</nav><span className={`system-state ${configured?'ready':''}`}><i/>{configured?'Systems ready':'Setup needed'}</span></header>
    {view==='sources'?<Sources sources={sources} configured={configured} events={events} onRefresh={refresh}/>:!profile||view==='profile'?<ProfileForm profile={profile} onSaved={(saved)=>{setProfile(saved);void refresh();setView('sources')}}/>:view==='discover'?<Discovery profile={profile} items={items} onSources={()=>go('sources')}/>:<Guide items={items}/>}
  </div>;
}
