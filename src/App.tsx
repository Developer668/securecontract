...
function Guide({items}:{items:FundingOpportunity[]}){
  ...
  const send=async()=>{const prompt=question.trim();if(!prompt||busy)return;setQuestion('');setError('');setBusy(true);setMessages((current)=>[...current,{id:crypto.randomUUID(),role:'user',content:prompt}]);try{const response=await fetch('/api/funding/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:prompt,opportunityIds:[]})});const body=await response.json() as {answer?:string;error?:string;evidenceIds?:string[];opportunityIds?:string[];recordsSearched?:number;recordsRead?:number};if(!response.ok)throw new Error(body.error??'The funding specialist is unavailable');setMessages((current)=>[...current,{id:crypto.randomUUID(),role:'assistant',content:body.answer??'The guide could not answer.',evidenceIds:body.evidenceIds,opportunityIds:body.opportunityIds,recordsRead:body.recordsRead,recordsSearched:body.recordsSearched}]);}catch(caught){setError(caught instanceof Error?caught.message:'The funding specialist is unavailable')}finally{setBusy(false)}};
