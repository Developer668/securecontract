...
function Guide({items}:{items:FundingOpportunity[]}){
  ...
  const send=async()=>{const prompt=question.trim();if(!prompt||busy)return;setQuestion('');setBusy(true);setMessages((current)=>[...current,{id:crypto.randomUUID(),role:'user',content:prompt}]);const response=await fetch('/api/funding/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:prompt,opportunityIds:[]})});const body=await response.json() as {answer?:string;error?:string;evidenceIds?:string[];opportunityIds?:string[];recordsSearched?:number;recordsRead?:number};setMessages((current)=>[...current,{id:crypto.randomUUID(),role:'assistant',content:body.answer??body.error??'The guide could not answer.',evidenceIds:body.evidenceIds,opportunityIds:body.opportunityIds,recordsRead:body.recordsRead,recordsSearched:body.recordsSearched}]);setBusy(false)};
  return <main className="chat-page"><header><Brain/><div><strong>FundingSecured Guide</strong><small>Minimax M3 on NVIDIA NIM · persisted grant JSON</small></div></header><section>{messages.map((message)=><article key={message.id} className={message.role}><span>{message.role==='assistant'?<Logo/>:'You'}</span><div><p>{message.content}</p>{message.opportunityIds?.length?<div className="grant-links">{message.opportunityIds.map((id)=>{const item=opportunities.get(id);return item?<a key={id} href={item.detailUrl} target="_blank" rel="noreferrer"><strong>{item.title}</strong><small>{item.funder} · {item.amountText}</small></a>:null})}</div>:null}{message.evidenceIds?.length?<footer>{message.evidenceIds.map((id,index)=>{const entry=evidence.get(id);return entry?<a key={id} href={entry.sourceUrl} target="_blank" rel="noreferrer">Evidence {index+1} · {entry.field}</a>:null})}</footer>:null}{message.recordsSearched?<small className="search-accountability">Searched {message.recordsSearched} saved records · read {message.recordsRead} full JSON records</small>:null}</div></article>)}{busy&&<p className="thinking"><SpinnerGap className="spin"/>Searching the portfolio and reading full grant JSON…</p>}</section><form onSubmit={(e)=>{e.preventDefault();void send()}}><textarea aria-label="Ask FundingSecured" value={question} onChange={(e)=>setQuestion(e.target.value)} placeholder={items.length?'Ask which grants fit, why, and what to do next…':'Run sources before asking about grants…'} disabled={!items.length}/><button disabled={!items.length||busy||question.trim().length<2}><PaperPlaneTilt/></button></form></main>;
}
