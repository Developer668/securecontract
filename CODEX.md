...
The CanadaBuys daily open-tender CSV (`canada-canadabuys`) plus the NYC (`us-nyc-current-bids`), Montgomery County (`us-montgomery-solicitations`), and San Francisco (`us-san-francisco-bids`) open-data APIs are registered public-source collectors. They have no fabricated Bright Data Collector IDs and run through the same validation and publication boundary. The Chicago source is warning-only while its official endpoint is unavailable; preserve its last-known-good data.
