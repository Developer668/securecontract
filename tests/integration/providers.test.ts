...
describe('server integrations',()=>{
  ...
  it('sends conversation history before the current question',async()=>{const fetcher=vi.fn().mockResolvedValue(new Response(JSON.stringify({choices:[{message:{content:JSON.stringify({answer:'The second record closes first.',evidenceFields:[],draft:false})}}]}),{status:200}));const provider=new NvidiaNimProvider({apiKey:'secret',baseUrl:'https://nim.example/v1',model:'model'},fetcher);await provider.chat({question:'Which closes first?',history:[{role:'user',content:'Compare these two records.'},{role:'assistant',content:'Here is a comparison.'}],opportunity:opportunities[0]});const payload=JSON.parse(String(fetcher.mock.calls[0]?.[1]?.body)) as {messages?:Array<{role:string}>};expect(payload.messages?.map(({role})=>role)).toEqual(['system','user','assistant','user']);});
});
